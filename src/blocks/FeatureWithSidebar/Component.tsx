// blocks/FeatureWithSidebar/Component.tsx
'use client'
...
export default function FeatureWithSidebarComponent() {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        // Intento 1: últimos 5 publicados por publishedAt
        ...
        // Fallback: si no hay publicados, tomar últimos 5 por createdAt
        if (!docs.length) {
        ...
        if (docs.length) {
          setLeftPost(docs[0])
          // Derecha: siguientes 4 (sin repetir el de la izquierda)
  ...
  return (
    <section className="px-4 md:px-[20%]">
      ...
      {/* Mobile: 1 columna (derecha abajo). md+: 2:1 con grid de 3 columnas */}
        ...
        <div className="md:col-span-2">
          <Link href={leftUrl} className="block no-underline">
            <div className="relative overflow-hidden rounded-xl bg-[#e9eef2]">
              {/* Mantener 16:9 sin plugin aspect */}
            ...
            <h2 className="mt-4 font-extrabold leading-[1.02] text-[clamp(28px,3.2vw,56px)] text-[#0e1f28]">
              {leftTitle}
            </h2>
          </Link>
        </div>
